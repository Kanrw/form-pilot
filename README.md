...
- 守卫**失败关闭**，但只对"本该有档案却没有"生效：`private/` 目录在、档案却读不出来
  （改名 / 误删 / 坏 JSON）→ 拒绝放行；`private/` 根本不在（新克隆、贡献者机器）→
  降级为模式判据并打告警，不会让你一个提交都做不了。
- `--staged` / `--history` 是**扫描范围**，`--patterns` 是**判据**（"不要求 private/"），两者可叠加。
