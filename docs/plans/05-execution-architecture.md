...
**做**：Moka 单站点端到端——**上传简历（必填文件槽位，且它把经历行 materialize 出来；
等解析稳定后，其解析结果一律不用，见 `AGENTS.md §七`）** → 注入引擎 → 扫描 → 字段映射确认 →
批量填文本 → 逐个下拉 → 按需加经历行 → 全量回读 → 停在提交前。
加上可分发形态（skill 包）与回归测试。
...
**不做** `nearest()` 几何归属：残留菜单本就落在 `before` 里被差集排除，找不到可命名的失败（§2.3）。

> **2026-09-22 部分推翻（真实投递实测）**：`closeAllMenus()` 当时被一并否掉，理由是
> "残留菜单落在 `before` 里被差集排除"。差集确实能排除它作为**候选**，但挡不住
> "点击已打开的触发器 = 关掉它"这个副作用 —— 一次失败留下残留菜单后，同一字段
> **再也填不上**，报错还一直是 `menu-not-open`（页面其实是好的），排查方向被完全带偏。
> 已实现最小版：`closeMenus()` 在每次 `openMenuFor` 前跑，只在真有残留时等待，干净页面零开销。
> `nearest()` 仍不做 —— 它至今没有可命名的失败。
...
> **本节已被 `AGENTS.md §六` 取代**（2026-09-22）：本节写于 `fillDate` / `fillMonthRange` 实现之前，
> 缺这两个方法，且 §二 的"不做 R4"一行也已随 §2.4 的解锁失效。
> 当前接口的**唯一事实源是 `AGENTS.md §六`**，本节保留作为"当初为什么这么定"的依据。
